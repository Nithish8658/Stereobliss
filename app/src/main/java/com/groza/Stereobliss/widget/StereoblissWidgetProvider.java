...
package com.groza.Stereobliss.widget;
...
public class
StereoblissWidgetProvider extends AppWidgetProvider {
    private static final String TAG = StereoblissWidgetProvider.class.getSimpleName();
    ...
    private synchronized void setWidgetContent(@NonNull NowPlayingInformation info, Context context) {
        ...
        RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.widget_stereobliss);
        ...
        // set button actions
        // Main action
        Intent mainIntent = new Intent(context, StereoblissMainActivity.class);
        ...
        if (nowPlaying) {
            ...
            mainIntent.putExtra(StereoblissMainActivity.MAINACTIVITY_INTENT_EXTRA_REQUESTEDVIEW, StereoblissMainActivity.REQUESTEDVIEW.NOWPLAYING.ordinal());
        }
        ...
        AppWidgetManager.getInstance(context).updateAppWidget(new ComponentName(context, StereoblissWidgetProvider.class), views);
    }

    private static class CoverReceiver implements CoverBitmapLoader.CoverBitmapReceiver {
        private final WeakReference<Context> mContext;
        private final WeakReference<StereoblissWidgetProvider> mProvider;

        CoverReceiver(final Context context, final StereoblissWidgetProvider provider) {
        ...
        public void receiveAlbumBitmap(Bitmap bm) {
            ...
            final StereoblissWidgetProvider provider = mProvider.get();
