...
public class MusicLibraryHelper {
    ...
    public static long getArtistIDFromName(final String artistName, final Context context) {
        ...
        final String where = MediaStoreProjections.ProjectionArtists.ARTIST + "=?";

        final String orderBy = MediaStoreProjections.ProjectionArtists.ARTIST + " COLLATE NOCASE";

        final Cursor cursor = PermissionHelper.query(context, MediaStore.Audio.Artists.EXTERNAL_CONTENT_URI, MediaStoreProjections.ProjectionArtists.PROJECTION, where, whereVal, orderBy);

        if (cursor != null) {
            if (cursor.moveToFirst()) {

                artistId = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionArtists.ID));
            }
    ...
    public static AlbumModel createAlbumModelFromId(final long albumId, final Context context) {
        ...
        final String where = MediaStoreProjections.ProjectionAlbums.ID + "=?";

        final Cursor cursor = PermissionHelper.query(context, MediaStore.Audio.Albums.EXTERNAL_CONTENT_URI, MediaStoreProjections.ProjectionAlbums.PROJECTION, where, whereVal, null);

        AlbumModel albumModel = null;
        if (cursor != null) {
            if (cursor.moveToFirst()) {

                final String albumTitle = cursor.getString(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionAlbums.ALBUM));
                final String albumArt = cursor.getString(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionAlbums.ALBUM_ART));
                final String artistTitle = cursor.getString(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionAlbums.ARTIST));
        ...
        if (albumId != -1) {
            ...
            if (cursor != null) {
                if (cursor.moveToFirst()) {
                    ...
                    retrievedAlbumId = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionAlbums.ID));
                }
            ...
            final String where = MediaStoreProjections.ProjectionAlbums.ALBUM + "=? AND " + MediaStoreProjections.ProjectionAlbums.ARTIST + "=?";
    ...
    public static List<TrackModel> getTracksForAlbum(final long albumId, final String orderKey, final Context context) {
        ...
        final String where = MediaStoreProjections.ProjectionTracks.ALBUM_ID + "=? AND " + MediaStoreProjections.ProjectionTracks.IS_MUSIC + "=?";

        String orderBy;

        if (orderKey.equals(context.getString(R.string.pref_album_tracks_sort_number_key))) {
            orderBy = MediaStoreProjections.ProjectionTracks.TRACK;
        } else if (orderKey.equals(context.getString(R.string.pref_album_tracks_sort_name_key))) {
            orderBy = MediaStoreProjections.ProjectionTracks.DISPLAY_NAME;
        } else {
            orderBy = MediaStoreProjections.ProjectionTracks.TRACK;
        }

        final Cursor cursor = PermissionHelper.query(context, MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, MediaStoreProjections.ProjectionTracks.PROJECTION, where, whereVal, orderBy);

        if (cursor != null) {
            // get all tracks on the current album
            if (cursor.moveToFirst()) {
                do {
                    final String trackName = cursor.getString(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.TITLE));
                    final long duration = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.DURATION));
                    final int number = cursor.getInt(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.TRACK));
                    final String artistName = cursor.getString(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.ARTIST));
                    final String albumName = cursor.getString(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.ALBUM));
                    final long artistId = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.ARTIST_ID));
                    final long id = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.ID));
    ...
    public static List<TrackModel> getTracksForArtist(final long artistId, final String albumOrderKey, final String trackOrderKey, final Context context) {
        ...
        if (albumOrderKey.equals(context.getString(R.string.pref_artist_albums_sort_name_key))) {
            orderBy = MediaStoreProjections.ProjectionAlbums.ALBUM;
        } else if (albumOrderKey.equals(context.getString(R.string.pref_artist_albums_sort_year_key))) {
            orderBy = MediaStoreProjections.ProjectionAlbums.FIRST_YEAR;
        } else {
            orderBy = MediaStoreProjections.ProjectionAlbums.ALBUM;
        }

        Cursor cursor = PermissionHelper.query(context, MediaStore.Audio.Artists.Albums.getContentUri("external", artistId),
                new String[]{MediaStoreProjections.ProjectionAlbums.ALBUM, MediaStoreProjections.ProjectionAlbums.ID, MediaStoreProjections.ProjectionAlbums.FIRST_YEAR}, "", null, orderBy + " COLLATE NOCASE");

        if (cursor != null) {
            if (cursor.moveToFirst()) {
                do {
                    long albumId = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionAlbums.ID));
    ...
    public static List<TrackModel> getTracksForPlaylist(final long playlistId, final Context context) {
        ...
        final Cursor cursor = PermissionHelper.query(context, MediaStore.Audio.Playlists.Members.getContentUri("external", playlistId), MediaStoreProjections.ProjectionPlaylistTracks.PROJECTION, "", null, "");

        if (cursor != null) {
            // get all tracks of the playlist
            if (cursor.moveToFirst()) {
                do {
                    final String trackName = cursor.getString(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionPlaylistTracks.TITLE));
                    final long duration = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionPlaylistTracks.DURATION));
                    final int number = cursor.getInt(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionPlaylistTracks.TRACK));
                    final String artistName = cursor.getString(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionPlaylistTracks.ARTIST));
                    final long artistId = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionPlaylistTracks.ARTIST_ID));
                    final String albumName = cursor.getString(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionPlaylistTracks.ALBUM));
                    final long albumId = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionPlaylistTracks.ALBUM_ID));
                    final long id = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionPlaylistTracks.AUDIO_ID));
    ...
    public static List<AlbumModel> getRecentAlbums(final Context context) {
        ...
        final String where = MediaStoreProjections.ProjectionTracks.IS_MUSIC + "=? AND " + MediaStoreProjections.ProjectionTracks.DATE_ADDED + ">?";

        final Cursor recentTracksCursor = PermissionHelper.query(context, MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, new String[]{MediaStoreProjections.ProjectionTracks.ALBUM_ID, MediaStoreProjections.ProjectionTracks.DATE_ADDED}, where, whereVal, MediaStoreProjections.ProjectionTracks.ALBUM_ID);

        // get all albums
        final Cursor albumsCursor = PermissionHelper.query(context, MediaStore.Audio.Albums.EXTERNAL_CONTENT_URI, MediaStoreProjections.ProjectionAlbums.PROJECTION, "", null, MediaStoreProjections.ProjectionAlbums.ID);

        if (recentTracksCursor != null && albumsCursor != null) {
            if (recentTracksCursor.moveToFirst() && albumsCursor.moveToFirst()) {

                final int albumIdColumnIndex = albumsCursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionAlbums.ID);
                final int albumTitleColumnIndex = albumsCursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionAlbums.ALBUM);
                final int imagePathColumnIndex = albumsCursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionAlbums.ALBUM_ART);
                final int artistTitleColumnIndex = albumsCursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionAlbums.ARTIST);
                final int numberOfSongsColumnIndex = albumsCursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionAlbums.NUMER_OF_SONGS);

                final int recentTracksAlbumIdColumnIndex = recentTracksCursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.ALBUM_ID);
                final int recentTracksDateAddedColumnIndex = recentTracksCursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.DATE_ADDED);
    ...
    public static List<TrackModel> getRecentTracks(final Context context) {
        ...
        final Cursor cursor = PermissionHelper.query(context, MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, MediaStoreProjections.ProjectionTracks.PROJECTION, where, whereVal, MediaStoreProjections.ProjectionTracks.ALBUM_ID);
        ...
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                ...
                do {
                    ...
                    final long artistId = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.ARTIST_ID));
                    final String albumName = cursor.getString(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.ALBUM));
                    final long albumId = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.ALBUM_ID));
                    final long id = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.ID));
    ...
    public static List<TrackModel> getAllTracks(final String filterString, final Context context) {
        ...
        final String where = MediaStoreProjections.ProjectionTracks.IS_MUSIC + "=?";

        final Cursor cursor = PermissionHelper.query(context, MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, MediaStoreProjections.ProjectionTracks.PROJECTION, where, whereVal, MediaStoreProjections.ProjectionTracks.TITLE + " COLLATE NOCASE");
    ...
    public static List<AlbumModel> getAllAlbums(final Context context) {
        ...
        final Cursor cursor = PermissionHelper.query(context, MediaStore.Audio.Albums.EXTERNAL_CONTENT_URI, MediaStoreProjections.ProjectionAlbums.PROJECTION, "", null, MediaStoreProjections.ProjectionAlbums.ALBUM + " COLLATE NOCASE");

        if (cursor != null) {
            if (cursor.moveToFirst()) {
                final int albumTitleColumnIndex = cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionAlbums.ALBUM);
                final int imagePathColumnIndex = cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionAlbums.ALBUM_ART);
                final int artistTitleColumnIndex = cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionAlbums.ARTIST);
                final int albumIdColumnIndex = cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionAlbums.ID);
    ...
    public static List<AlbumModel> getAllAlbumsForArtist(final long artistId, final String orderKey, final Context context) {
        ...
        Cursor cursor = PermissionHelper.query(context, MediaStore.Audio.Artists.Albums.getContentUri("external", artistId), MediaStoreProjections.ProjectionAlbums.PROJECTION, "", null, orderBy + " COLLATE NOCASE");
    ...
    public static List<ArtistModel> getAllArtists(final boolean showAlbumArtistsOnly, final Context context) {
        ...
        if (!showAlbumArtistsOnly) {
            ...
            // get all artists
            final Cursor cursor = PermissionHelper.query(context, MediaStore.Audio.Artists.EXTERNAL_CONTENT_URI, MediaStoreProjections.ProjectionArtists.PROJECTION,
                    "", null, MediaStoreProjections.ProjectionArtists.ARTIST + " COLLATE NOCASE ASC");

            if (cursor != null) {

                if (cursor.moveToFirst()) {
                    final int artistTitleColumnIndex = cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionArtists.ARTIST);
                    final int artistIdColumnIndex = cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionArtists.ID);
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                ...
                final Cursor cursor = PermissionHelper.query(context, MediaStore.Audio.Albums.EXTERNAL_CONTENT_URI, MediaStoreProjections.ProjectionAlbums.PROJECTION,
                        "", null, MediaStoreProjections.ProjectionAlbums.ARTIST + " COLLATE NOCASE ASC");

                if (cursor != null) {

                    if (cursor.moveToFirst()) {

                        final int albumArtistTitleColumnIndex = cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionAlbums.ARTIST);
                        final int albumIdColumnIndex = cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionAlbums.ARTIST_ID);
                ...
                final Cursor cursor = PermissionHelper.query(context, MediaStore.Audio.Albums.EXTERNAL_CONTENT_URI, new String[]{MediaStoreProjections.ProjectionAlbums.ARTIST, MediaStoreProjections.ProjectionAlbums.ALBUM},
                        MediaStoreProjections.ProjectionAlbums.ARTIST + "<>\"\" ) GROUP BY (" + MediaStoreProjections.ProjectionAlbums.ARTIST, null, MediaStoreProjections.ProjectionAlbums.ARTIST + " COLLATE NOCASE ASC");

                if (cursor != null) {

                    if (cursor.moveToFirst()) {

                        int albumArtistTitleColumnIndex = cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionAlbums.ARTIST);
    ...
    public static List<PlaylistModel> getAllPlaylists(final Context context) {
        ...
        final Cursor cursor = PermissionHelper.query(context, MediaStore.Audio.Playlists.EXTERNAL_CONTENT_URI, MediaStoreProjections.ProjectionPlaylists.PROJECTION, "", null, MediaStoreProjections.ProjectionPlaylists.NAME);

        if (cursor != null) {

            if (cursor.moveToFirst()) {
                final int playlistTitleColumnIndex = cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionPlaylists.NAME);
                final int playlistIdColumnIndex = cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionPlaylists.ID);
    ...
    public static Set<String> getTrackStorageLocationsForAlbum(final long albumId, final Context context) {
        ...
        final String orderBy = MediaStoreProjections.ProjectionTracks.TRACK;
        ...
        if (cursor != null) {
            ...
            if (cursor.moveToFirst()) {
                do {
                    final String url = cursor.getString(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.DATA));
    ...
    static TrackModel getTrackForUri(final Uri uri, final Context context) {
        ...
        String where = MediaStoreProjections.ProjectionTracks.DATA + "=?";
        ...
        if (uriScheme != null && uriScheme.equals("content")) {
            ...
            if (parts != null && parts.length > 1) {
                if (parts[0].equals("audio")) {
                    ...
                    where = MediaStoreProjections.ProjectionTracks.ID + "=?";
                } else {
                    whereVal = new String[]{"%" + parts[1]};
                    where = MediaStoreProjections.ProjectionTracks.DATA + " LIKE ?";
                }
            }
        }

        // lookup the current file in the media db
        final Cursor cursor = PermissionHelper.query(context, MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, MediaStoreProjections.ProjectionTracks.PROJECTION, where, whereVal, MediaStoreProjections.ProjectionTracks.TRACK);

        if (cursor != null) {
            if (cursor.moveToFirst()) {
                final String title = cursor.getString(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.TITLE));
                final long duration = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.DURATION));
                final int no = cursor.getInt(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.TRACK));
                final String artist = cursor.getString(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.ARTIST));
                final long artistId = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.ARTIST_ID));
                final String album = cursor.getString(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.ALBUM));
                final long albumId = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.ALBUM_ID));
                final long id = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStoreProjections.ProjectionTracks.ID));
    ...
    static List<FileModel> getMediaFilesForPath(final String basePath, final Context context) {
        ...
        final String where = MediaStoreProjections.ProjectionTracks.DATA + " LIKE ?";
